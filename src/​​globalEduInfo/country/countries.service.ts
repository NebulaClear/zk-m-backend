  ...
  export class CountryService {
    ...
    async create(dto: CreateCountryDto): Promise<Country> {
      const existing = await this.countryRepo.findOne({
        where: [{ country_id: dto.country_id }, { country_ch_name: dto.country_ch_name }, { country_en_name: dto.country_en_name }]
      });
      ...
      return this.countryRepo.manager.transaction(async transactionalEntityManager => {
        ...
        if (dto.currencies?.length) {
          ...
          await transactionalEntityManager.save(CountryCurrency, currencies.map(c => ({
        ...
        if (dto.languages?.length) {
          ...
          await transactionalEntityManager.save(CountryLanguage, languages.map(l => ({
            country_id: savedCountry.country_id,
        ...
        if (dto.timezones?.length) {
          ...
          await transactionalEntityManager.save(CountryTimeZone, timezones.map(t => ({
            country_id: savedCountry.country_id,
    ...
    async update(id: string, dto: UpdateCountryDto): Promise<Country> {
      ...
      return this.countryRepo.manager.transaction(async transactionalEntityManager => {
        ...
        // 处理关联更新逻辑（示例：货币更新）
        if (dto.currencies) {
          await transactionalEntityManager.delete(CountryCurrency, { country_id: id });
          const currencies = await this.currencyRepo.find({
            where: { currency_code: In(dto.currencies) }
          });
          await transactionalEntityManager.save(CountryCurrency, currencies.map(c => ({
